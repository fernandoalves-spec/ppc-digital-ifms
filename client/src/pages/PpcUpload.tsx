...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  extracted: { label: "ExtraÃ­do", color: "bg-amber-100 text-amber-700" },
...
export default function PpcUploadPage() {
  ...
  const extractMutation = trpc.ppc.extract.useMutation({
    onSuccess: (data) => {
      ...
      // Campus selecionado pelo usuÃ¡rio Ã© sempre preservado
      const selectedCampus = (campuses as any[]).find((c: any) => c.id === selectedCampusId);
      if (selectedCampus) {
        d.campusName = selectedCampus.name;
      }
      toast.success(`ExtraÃ§Ã£o concluÃ­da! ${d.subjects?.length ?? 0} disciplinas encontradas.`);
      setEditedData(JSON.parse(JSON.stringify(d)));
      // Marcar todas as disciplinas como selecionadas por padrÃ£o
      const selected = new Set<number>();
      d.subjects.forEach((_, i) => selected.add(i));
      setSelectedSubjects(selected);
      setShowReview(true);
    },
    onError: (e) => toast.error(`Erro na extraÃ§Ã£o: ${e.message}`),
  });

  const applyMutation = trpc.ppc.applyExtraction.useMutation({
    onSuccess: () => {
      utils.ppc.list.invalidate();
      toast.success("ImportaÃ§Ã£o concluÃ­da! Campus, curso e disciplinas cadastrados.");
      setShowReview(false);
      setEditedData(null);
      setApplyDocId(null);
  ...
  const handleFileSelect = (f: File) => {
    if (f.type !== "application/pdf") return toast.error("Apenas arquivos PDF sÃ£o aceitos.");
    if (f.size > 20 * 1024 * 1024) return toast.error("Arquivo muito grande. MÃ¡ximo 20MB.");
    setFile(f);
  };
  ...
  const handleOpenReview = (doc: any) => {
    ...
    if (!d) return toast.error("Dados de extraÃ§Ã£o nÃ£o encontrados.");
    setEditedData(JSON.parse(JSON.stringify(d)));
    setApplyDocId(doc.id);
    const selected = new Set<number>();
    d.subjects.forEach((_, i) => selected.add(i));
  ...
  const handleApply = () => {
    ...
    if (!selectedCampusId) return toast.error("Campus nÃ£o selecionado. Volte e selecione o campus.");
    ...
    applyMutation.mutate({
      ...
      // Campus Ã© imutÃ¡vel: usa sempre o campusId definido pelo usuÃ¡rio
  ...
  return (
    <div className="page-stack p-3 md:p-6">
      <PageHeader badge="Fluxo operacional" title="Upload de PPC" description="Envie um PDF de PPC para extrair curso, campus, disciplinas e areas de ensino com apoio de IA." />
          ...
          >
            ...
            {file ? (
              ...
              <div className="flex flex-col items-center gap-2">
                ...
                <p className="text-xs text-slate-600">MÃ¡ximo 20MB Â· Apenas PDF</p>
              </div>
            )}
          </div>
          {file && (
        ...
        <Card className="border-green-200 bg-green-50/30">
          <CardHeader className="pb-3">
            ...
            <p className="text-sm text-slate-600 mt-1">
              Revise as informaÃ§Ãµes extraÃ­das pela IA. Marque/desmarque disciplinas e edite os campos necessÃ¡rios antes de importar.
            </p>
          </CardHeader>
          <CardContent className="space-y-5">
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
              <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                <GraduationCap className="w-4 h-4 text-green-600" /> InformaÃ§Ãµes do Curso
              </h3>
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-2.5 flex items-start gap-2">
                <AlertCircle className="w-4 h-4 text-blue-600 mt-0.5 shrink-0" />
                <p className="text-xs text-blue-800">
                  O <strong>campus foi definido por vocÃª</strong> e nÃ£o pode ser alterado. Se o curso nÃ£o existir no sistema, serÃ¡ <strong>criado automaticamente</strong> ao aplicar.
                </p>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <Badge variant="outline" className="text-[10px] px-1.5 py-0 text-green-700 border-green-200 bg-green-50 shrink-0 whitespace-nowrap">Definido por vocÃª</Badge>
                  </div>
                </div>
                <div className="space-y-1">
                ...
                <div className="space-y-1">
                  ...
                  <Select value={editedData.courseType} onValueChange={(v) => setEditedData({ ...editedData, courseType: v })}>
                    ...
                    <SelectContent>
                      {["TÃ©cnico", "Subsequente", "GraduaÃ§Ã£o", "FIC", "PÃ³s-graduaÃ§Ã£o"].map(t => (
                ...
                <div className="space-y-1">
                  <Label className="text-xs text-slate-500">DuraÃ§Ã£o (sem.)</Label>
            ...
            <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-700 flex items-center gap-2">
                  <BookOpen className="w-4 h-4 text-green-600" /> Disciplinas ExtraÃ­das
                </h3>
                <div className="flex items-center gap-3">
                    ...
                    .map(([sem, subs]) => (
                      <div key={sem}>
                        <div className="flex items-center gap-2 mb-2 sticky top-0 bg-white z-10 py-1">
                          ...
                          <span className="text-xs font-bold text-slate-500 bg-white px-2">{sem}Âº SEMESTRE</span>
                          <div className="h-px flex-1 bg-slate-200" />
                        </div>
                        <div className="space-y-1">
                          {subs.map((s) => {
                            ...
                            return (
                              <div key={s._idx} className={`rounded-lg border transition-all ${isSelected ? "border-green-200 bg-green-50/50" : "border-slate-100 bg-slate-50/50 opacity-60"}`}>
                                {/* Linha principal: checkbox + nome + badges + aÃ§Ãµes */}
                                <div className="flex items-center gap-3 px-3 py-2.5">
                                  ...
                                  <div
                                    ...
                                    role="button"
                                    tabIndex={0}
                                    onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setExpandedSubject(isExpanded ? null : s._idx); } }}
                                  ...
                                  >
                                    ...
                                    <div className="flex flex-wrap items-center gap-1.5 mt-0.5">
                                      ...
                                      {s.suggestedArea && s.suggestedArea !== "NÃ£o identificada" && (
                                  ...
                                  <div className="flex items-center gap-1 shrink-0">
                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-blue-600" onClick={() => setEditingIdx(isEditing ? null : s._idx)}
                                      aria-label={`Editar disciplina ${s.name}`}>
                                      <Pencil className="w-3.5 h-3.5" />
                                    </Button>
                                    <Button variant="ghost" size="icon" className="h-7 w-7 text-slate-400 hover:text-red-600" onClick={() => removeSubject(s._idx)}
                                      aria-label={`Remover disciplina ${s.name}`}>
                                      <Trash2 className="w-3.5 h-3.5" />
                                    </Button>
                                    <button onClick={() => setExpandedSubject(isExpanded ? null : s._idx)} className="text-slate-500 hover:text-slate-700 p-1" aria-label={isExpanded ? "Recolher detalhes da disciplina" : "Expandir detalhes da disciplina"} aria-expanded={isExpanded}>
                                ...
                                {isEditing && (
                                  <div className="px-3 pb-3 pt-1 border-t border-slate-100 bg-white space-y-3">
                                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                      ...
                                      <div className="space-y-1">
                                        <Label className="text-[11px] text-slate-500">Carga HorÃ¡ria</Label>
                                    ...
                                    <div className="space-y-1">
                                      <Label className="text-[11px] text-slate-500">Ãrea de Ensino</Label>
                                      {campusAreas.length > 0 ? (
                                        ...
                                        >
                                          ...
                                          <SelectContent>
                                            <SelectItem value="__none__">Sem Ã¡rea definida</SelectItem>
                                        ...
                                        <Input value={s.suggestedArea} onChange={(e) => updateSubjectField(s._idx, "suggestedArea", e.target.value)} className="h-8 text-sm" placeholder="Sem Ã¡reas vinculadas ao campus" />
                                      )}
                                    </div>
                                    ...
                                    <div className="space-y-1">
                                      <Label className="text-[11px] text-slate-500">ReferÃªncias BibliogrÃ¡ficas</Label>
                                    ...
                                    <div className="flex justify-end">
                                      <Button size="sm" variant="outline" className="h-7 text-xs" onClick={() => setEditingIdx(null)}>
                                        <Check className="w-3 h-3 mr-1" /> Fechar EdiÃ§Ã£o
                                      </Button>
                                    </div>
                                  </div>
                                )}

                                {/* Expandir ementa/referÃªncias (somente visualizaÃ§Ã£o) */}
                                {isExpanded && !isEditing && (
                                  <div className="px-3 pb-3 pt-1 border-t border-slate-100 bg-slate-50/50 space-y-2">
                                    ...
                                    ) : <p className="text-xs text-slate-400 italic">Ementa nÃ£o extraÃ­da</p>}
                                    {s.bibliography ? (
                                      <div>
                                        <p className="text-[11px] font-semibold text-slate-500 mb-0.5">ReferÃªncias</p>
                                        <p className="text-xs text-slate-700 bg-white rounded p-2 border border-slate-100 whitespace-pre-wrap">{s.bibliography}</p>
                                      </div>
                                    ) : <p className="text-xs text-slate-400 italic">ReferÃªncias nÃ£o extraÃ­das</p>}
                                  </div>
                                )}
                              </div>
                            );
            ...
            {/* BotÃ£o de Aplicar â€” sticky para nÃ£o sobrepor */}
            <div className="sticky bottom-0 z-20 flex items-center justify-between bg-white rounded-lg border border-green-200 shadow-lg p-4 mt-4">
              <div className="text-sm text-slate-600">
                <strong className="text-green-700">{selectedSubjects.size}</strong> disciplinas selecionadas para importaÃ§Ã£o
              </div>
